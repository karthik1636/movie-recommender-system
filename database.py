...
class MovieDatabase:
    ...
    def close(self):
        """Close the database connection - no longer needed with per-operation connections"""
        pass
    ...
    def get_user_ratings(self, user_id: int) -> List[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT movie_id, rating FROM user_ratings WHERE user_id = ?", (user_id,)
        )
    ...
    def get_movie_by_id(self, movie_id: int) -> Optional[Dict]:
        ...
        cursor = conn.cursor()
        cursor.execute(
            "SELECT title, genres FROM movies WHERE movieId = ?", (movie_id,)
        )
        row = cursor.fetchone()
        conn.close()
        if row:
            return {"movieId": movie_id, "title": row[0], "genres": row[1]}
        return None
    ...
    def search_movies(self, query: str, limit: int = 10) -> list:
        ...
        conn.close()
        return [
            {"movieId": row[0], "title": row[1], "genres": row[2]} for row in results
        ]
