...
class LocalMovieRecommender:
    def __init__(self, user_item_matrix, movies_df, n_components=50):
        ...
        self.item_mapping = {item_id: idx for idx, item_id in enumerate(user_item_matrix.columns)}
...
class MovieRecommenderAPI:
    ...
    def get_movie_info(self, movie_id):
        ...
        try:
            movie_info = self.recommender.movies_df[self.recommender.movies_df['movieId'] == movie_id]
            if not movie_info.empty:
                return {
                    "movie_id": movie_id,
                    "title": movie_info.iloc[0]['title'],
    ...
    def get_available_movies(self, limit=10):
        ...
        try:
            movies = self.recommender.movies_df.head(limit)[['movieId', 'title', 'genres']].to_dict('records')
            return {
                "movies": movies,
                "count": len(movies),
    ...
    def search_movies(self, query, limit=10):
        ...
        try:
            query = query.lower()
            matching_movies = self.recommender.movies_df[
                self.recommender.movies_df['title'].str.lower().str.contains(query)
            ].head(limit)[['movieId', 'title', 'genres']].to_dict('records')
    ...
    def get_model_stats(self):
        ...
        try:
            return {
                ...
                "explained_variance_ratio": float(self.recommender.svd.explained_variance_ratio_.sum())
            }
        except Exception as e:
        ...
        try:
            ...
            if len(recommended_movies) < 3:
                ...
                for movie in fallback_movies:
                    if movie['title'] not in seen_titles and len(recommended_movies) < n_recommendations:
    ...
    def _extract_movie_titles(self, text):
        ...
        for line in lines:
            ...
            cleaned = re.sub(r'^(movie|film|title):\s*', '', cleaned, flags=re.IGNORECASE)
            ...
            if (len(cleaned) > 3 and 
                ...
                not cleaned.startswith('*') and
                'programming' not in cleaned.lower() and
                'code' not in cleaned.lower() and
                'function' not in cleaned.lower() and
                'algorithm' not in cleaned.lower() and
                'database' not in cleaned.lower()):
    ...
    def _extract_keywords(self, query):
        ...
        movie_keywords = [
            ...
            'adventure', 'fantasy', 'mystery', 'crime', 'war', 'western', 'musical',
            'documentary', 'animation', 'family', 'children', 'teen', 'adult',
            'romantic', 'funny', 'scary', 'exciting', 'sad', 'happy', 'love',
            'fighting', 'space', 'magic', 'detective', 'police', 'soldier',
            'cowboy', 'singing', 'real', 'cartoon', 'kid', 'young', 'grown'
        ]
        ...
        if not found_keywords:
            common_movies = [
                'inception', 'shawshank', 'titanic', 'star wars', 'matrix', 
                'die hard', 'jaws', 'godfather', 'pulp fiction', 'forrest gump',
                'goodfellas', 'fight club', 'interstellar', 'dark knight',
                'avatar', 'toy story', 'lion king', 'frozen', 'finding nemo'
            ]
        ...
        time_keywords = ['80s', '90s', '2000s', '2010s', '2020s', 'old', 'new', 'recent', 'classic']
        ...
        if not found_keywords:
            ...
            meaningful_words = [word for word in words if len(word) > 3 and word not in ['give', 'me', 'want', 'like', 'show', 'find', 'recommend']]
    ...
    def explain_recommendations(self, user_id, model="deepseek-coder"):
        ...
        try:
            ...
            movies_text = ", ".join([rec['title'] for rec in recommendations["recommendations"]])
...
def example_usage():
    ...
    print(api.ask_llm("Give me sci-fi thrillers from the 2010s", model="deepseek-coder", n_recommendations=5))
