...
for i, rec in enumerate(recommendations, 1):
    print(f"{i}. {rec['title']} (Genres: {rec['genres']}) - Score: {rec['similarity_score']:.4f}")
...
for i, movie in enumerate(similar_movies, 1):
    print(f"{i}. {movie['title']} (Genres: {movie['genres']}) - Score: {movie['similarity_score']:.4f}")
...
def calculate_coverage(recommender, user_item_matrix, n_recommendations=10):
    ...
    total_users = min(100, len(user_item_matrix.index))  # Sample 100 users for efficiency
