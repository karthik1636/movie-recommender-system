...
class Deployer:
    ...
    def _check_aws_credentials(self):
        ...
        try:
            ...
            self.logger.info(
                f"AWS credentials valid for account: {identity['Account']}"
            )
    ...
    def _run_aws_deployment(self):
        ...
        try:
            subprocess.run(
                ["bash", str(deploy_script)], cwd=self.project_root, check=True
            )
    ...
    def _check_application_health(self):
        ...
        try:
            ...
            response = requests.get(
                f"http://{self.config.app.host}:{self.config.app.port}", timeout=10
            )
