    ...
    **New AI Features:**
    ...
    if "error" not in users_data:
        ...
        selected_user = st.selectbox(
            "Select a user:", users_data["users"], format_func=lambda x: f"User {x}"
        )
            ...
            if st.button("Get Recommendations"):
                with st.spinner("Getting recommendations..."):
                    recommendations = recommender.get_user_recommendations(
                        selected_user, n_recommendations
                    )

                if "error" not in recommendations:
                    st.success(
                        f"Found {recommendations['count']} recommendations for User {selected_user}"
                    )

                    # Display recommendations
                    for i, rec in enumerate(recommendations["recommendations"], 1):
                        with st.expander(
                            f"{i}. {rec['title']} (Score: {rec['similarity_score']:.3f})"
                        ):
            ...
            if st.button("🤖 Explain Recommendations"):
                with st.spinner("AI is analyzing the recommendations..."):
                    explanation = recommender.explain_recommendations(
                        selected_user, model="deepseek-coder"
                    )
                ...
                if "error" not in explanation:
                    ...
                    for i, rec in enumerate(explanation["recommendations"], 1):
                        st.write(
                            f"{i}. **{rec['title']}** ({rec['genres']}) - Score: {rec['similarity_score']:.3f}"
                        )
                else:
    ...
    if search_query:
        ...
        if "error" not in search_results and search_results["count"] > 0:
            st.write(
                f"Found {search_results['count']} movies matching '{search_query}'"
            )

            # Movie selection
            movie_options = {
                f"{movie['title']} (ID: {movie['movieId']})": movie["movieId"]
            ...
            if st.button("Find Similar Movies"):
                ...
                if "error" not in similar_movies:
                    ...
                    for i, movie in enumerate(similar_movies["similar_movies"], 1):
                        with st.expander(
                            f"{i}. {movie['title']} (Score: {movie['similarity_score']:.3f})"
                        ):
                            st.write(f"**Genres:** {movie['genres']}")
                            st.write(f"**Movie ID:** {movie['movieId']}")
                            st.write(
                                f"**Similarity Score:** {movie['similarity_score']:.4f}"
                            )
                else:
