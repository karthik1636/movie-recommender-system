...
class PerformanceMonitor:
    ...
    def _monitor_system_resources(self, interval_seconds: int):
        ...
        while not self._stop_monitoring:
            try:
                ...
                self.metrics.record_metric("system.network_bytes_sent", network.bytes_sent)
                self.metrics.record_metric("system.network_bytes_recv", network.bytes_recv)
    ...
    def record_recommendation_request(self, user_id: int, algorithm: str, 
                                    num_recommendations: int, response_time: float):
        """Record recommendation request metrics"""
        self.metrics.record_metric("recommendations.response_time_ms", response_time * 1000, {
            "algorithm": algorithm,
            "user_id": str(user_id)
        })
        
        self.metrics.record_metric("recommendations.count", 1, {
            "algorithm": algorithm
        })
        
        self.metrics.record_metric("recommendations.num_results", num_recommendations, {
            "algorithm": algorithm
        })
        
        self.logger.log_recommendation(user_id, algorithm, num_recommendations, response_time)
    
    def record_llm_query(self, model: str, query: str, response_time: float, 
                        num_results: int, success: bool):
        """Record LLM query metrics"""
        self.metrics.record_metric("llm.response_time_ms", response_time * 1000, {
            "model": model,
            "success": str(success)
        })
        
        self.metrics.record_metric("llm.queries_count", 1, {
            "model": model,
            "success": str(success)
        })
        
        if success:
            self.metrics.record_metric("llm.results_count", num_results, {
                "model": model
            })
    ...
    def record_user_action(self, user_id: int, action: str, details: Optional[Dict] = None):
        ...
        self.metrics.record_metric("user.actions_count", 1, {
            "action": action,
            "user_id": str(user_id)
        })
    ...
    def record_model_performance(self, metric: str, value: float, algorithm: str = "svd"):
        """Record model performance metrics"""
        self.metrics.record_metric(f"model.{metric}", value, {
            "algorithm": algorithm
        })
...
def record_recommendation(user_id: int, algorithm: str, num_recommendations: int, response_time: float):
...
def record_llm_query(model: str, query: str, response_time: float, num_results: int, success: bool):
    """Quick function to record LLM query metrics"""
    get_performance_monitor().record_llm_query(model, query, response_time, num_results, success)
